...
Public dbSegurida As DAO.Database 'para programas con segurida.mdb
Public db As DAO.Database            ' Base de datos
...
Sub Mouse(caso As MousePointerConstants)
    Dim p As POINTAPI
...
Public Sub GetVentana(f As Form)
    With f
        ...
        .Height = GetIni(.Name & "-H", "", App.Path & "\Ventanas.ini", .Height)
        .Width = GetIni(.Name & "-W", "", App.Path & "\Ventanas.ini", .Width)
    End With
End Sub
    ...
    For Each oTabla In db.TableDefs
        If oTabla.SourceTableName <> "" And (Left$(oTabla.Connect, 1) = ";" Or LCase(Left$(oTabla.Connect, 10)) = "ms access;") Then
            ...
            If s <> "" Then
                ...
                ElseIf sNomMdb = "sgpaserv.mdb" Then
                    sPass2 = "XXXXXX"
                Else
                    sPass2 = sPass
                End If
...
Public Function BrowseForFolder(ByVal f_HWnd As Long, _
                                Optional lpTitle As Variant, Optional psRoot As String = "") As String
...
With lpbi
    ...
    .pidlRoot = psRoot 'comenzar a partir del Escritorio
